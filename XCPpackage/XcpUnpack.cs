namespace XCPpackage
{
    ...
    public class XcpUnpack : IDisposable
    {
        ...
        private void Inflate(FILE source, FILE dest)
        {
            ...
            ZlibStream zlibStream = new ZlibStream(source, CompressionMode.Decompress, true)
            {
                BufferSize = 16384,
            ...
            while (zlibStream.Read(buffer, 0, buffer.Length) != 0)
            {
                ...
                short num2 = (short)Math.Ceiling((_lastStream + (double)zlibStream.TotalIn) / _srcsize * 100.0);
                if (num1 != num2)
                {
                    num1 = num2;
                    this.OnUnpackProgressChanged(new ProgressChangedEventArgs(num2));
                    this.OnUnpackAndSplitProgressChanged(new ProgressChangedEventArgs(num2 / 2));
                }
            }
        ...
        private List<FileInfo> second_pass(FILE unpackedXcp, string outDir)
        {
            ...
            while (num1 < length)
            {
                if (!Directory.Exists(string.Format("{0}.data", filePath)))
                    Directory.CreateDirectory(string.Format("{0}.data", filePath));
                using (FILE file = new FILE(string.Format("{0}.data\\Data{1:D4}", filePath, num2), FileMode.Create, FileAccess.Write, FileShare.Read, 16384))
                {
                    for (int index = 0; index < 10404; ++index)
                    {
                        ...
                        num1 += count;
                        short num4 = (short)Math.Ceiling(num1 / (double)length * 100.0);
                        if (num4 != num3)
                        {
                            num3 = num4;
                            this.OnSplitProgressChanged(new ProgressChangedEventArgs(num4));
                            this.OnUnpackAndSplitProgressChanged(new ProgressChangedEventArgs(num4 / 2 + 50));
                        }
                    }
                    fileInfoList.Add(file.Fi);
                    ++num2;
